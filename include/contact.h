...
public:
  ...
  */
  ...
  /*! Sets the contact position wrt Body1 frame */
  void setPosition(position newLoc);
...
/*!	This class is meant for studing grasp quality when there is really no grasp, as
	...
	coordinate system. The reason is that we can have a grasp defined by many virtual
...
public:
	...
	//! Set the number of the finger that this contact is on
	void setFingerNum(int newFingerNum){mFingerNum = newFingerNum;}

	//! Set the number of the link that this contact is on
	void setLinkNum(int newLinkNum){mLinkNum = newLinkNum;}
};
...
public:
	...
	virtual ~VirtualContactOnObject ();
	void readFromFile(FILE * fp);
#ifdef ARIZONA_PROJECT_ENABLED
