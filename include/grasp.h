...
protected:
  ...
  //! Desired task wrench that the grasp wants to resist
  double taskWrench[6];
...
public:
  ...
  void                    getTaskWrench(double *w) const
  {
    if (w) memcpy(w,taskWrench,6*sizeof(double));
  }

  void                    setTaskWrench(double *w)
  {
    if (w) memcpy(taskWrench,w,6*sizeof(double));
  }
  ...
  int computeQuasistaticForcesAndTorques(Matrix *robotTau, Matrix * objWrench = NULL, double * objectiveValue = NULL);
