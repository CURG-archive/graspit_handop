...
signals:
  ...
  //! signal that tells world object to change threads
  void cloneWorldHand(Hand * h, Hand ** handptrptr);

  //! signal that acknowledges thread has been changed
  void worldClonedHand();

private slots:
  void cloneWorldHandSlot(Hand * h, Hand ** handptrptr);
...
public:	
  ...
  void emitWorldHandCloned(){emit worldClonedHand();}

  //  void emitWorldCloneHand(Hand * h){emit cloneWorldHand(t);


  //helper function to change threads of world and wait until its done.
  Hand * cloneHand(Hand * h);
  //! Slot that recieves command to change world thread
