...
protected:
 ...
 public:
  ...
  //! Serts the eigengrasps of this robot
  void setEigenGrasps(EigenGraspInterface * newEigenGrasps){mEigenGrasps = newEigenGrasps;}
...
//! Sets the name of this robot
  ...
  //! Add dof to the end of dofVec
  void addDOF(DOF * d){dofVec.push_back(d); ++numDOF;}
  ...
  //! Add a chain to the end of the chain vector
  void addChain(KinematicChain * c){chainVec.push_back(c); numChains+=1; numJoints += c->getNumJoints();}
...
//forward declarations for RobotTools
enum materialT;
...
namespace RobotTools{
  ...
  bool scaleHand(const std::vector<mat3> & scales, Hand * h);
  bool scaleChain(const mat3 scales[], Link * palm, KinematicChain * kc);

  void writeContactsBody(Hand * h, Body * b, QTextStream & os);
  void writeContactLinks(Hand * h, Body * b, QTextStream & os);
  void writeContacts(Hand * h, Body * b, QTextStream & os);
};
