...
public:
 ...
 Joint(KinematicChain *k) : DOFnum(0), owner(k), jointNum(-1), draggerAttached(false), dynamicsVal(0.0),
  ...
  /*! Set up the joint using values from an XML DOM read from the robot
	  configuration file.
  */
  virtual int initJointFromXml(const TiXmlElement* root, int jnum, int DOFOffset) =0;
  ...
  /*! Sets the constant joint offset value. */
  void setOffset(double newOffset){c = newOffset;}

  /*! Returns the minimum joint limit */
  double getMin() const {return minVal;}

  /*! Sets the minimum joint limit */
  void setMin(double newMinVal){minVal = newMinVal;};

  /*! Returns the maximum joint limit. */
  double getMax() const {return maxVal;}

  /*! Sets the maximum joint limit */
  void setMax(double newMaxVal){maxVal = newMaxVal;};
...
public:
  ...
  virtual int initJointFromXml(const TiXmlElement* root, int jnum, int DOFOffset);
...
public:
	...
	virtual int initJointFromXml(const TiXmlElement* root, int jnum, int);
	virtual int setVal(double q);
