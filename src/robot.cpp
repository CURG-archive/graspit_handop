...
k	to be the root of a hierarchy containing all the relevant
...
Robot::loadFromXml(const TiXmlElement* root,QString rootPath)
{
	...
	//commented out for eigenhand experiment
	//if (numDOF < 1) {
...
Hand::findInitialContact(double moveDist)
{
	...
	//create a vector of bodies in this hand
	std::vector <Body * > interestList;
...
bool RobotTools::scaleHand(const std::vector<mat3> & scales, Hand * h)
{ 
  if (h->getNumLinks() != scales.size())
    return false;

  RobotTools::scalePalm(scales[0], h);
  unsigned int scaleIterator = 1;

  for(int kcIterator = 0; kcIterator < h->getNumChains(); ++kcIterator){
    scaleChain(&scales[scaleIterator], h->getPalm(), h->getChain(kcIterator));
    scaleIterator += h->getChain(kcIterator)->getNumLinks();	       
  }
  return true;
}
...
bool RobotTools::scaleChain(const mat3 scales[], 
			    Link * palm,  KinematicChain * kc)
{  
  for(int linkIndex = 0; linkIndex < kc->getNumLinks(); ++linkIndex)
    {
    RobotTools::scaleLink(scales[linkIndex], kc->getLink(linkIndex));
    RobotTools::scaleJoint(scales[linkIndex], kc->getJoint(linkIndex), kc->getLink(linkIndex));
    if(linkIndex == 0)      
	kc->getOwner()->getWorld()->toggleCollisions(false, kc->getLink(linkIndex), palm);      
    else      
...
void RobotTools::scaleJoint(const mat3 & scale, Joint * j, Link * l){
  //mat3 currScale = normalizeScale(scale, getScalingNode(l->getIVGeomRoot()));
	j->getDH()->setA(j->getDH()->getA()*scale[0]);
	j->getDH()->setD(j->getDH()->getD()*scale[4]);
...
void scaleContactPosition(const mat3 & scale, Contact * c)
{
  position pos = c->getPosition();
  c->setPosition(scale*pos);
  std::cout << "VC position: " <<pos;
...
void RobotTools::scaleLink(const mat3 & scale, Link * l){
	...
	std::list<Contact *> vc = l->getVirtualContacts();
	for(std::list<Contact *>::iterator vcIter = vc.begin(); 
	    vcIter != vc.end(); ++vcIter)
	  scaleContactPosition(scale, *vcIter);
	l->redrawFrictionCones();
}
...
//outputs the contacts in the robot's frame
void RobotTools::writeContacts(Hand * h, Body * b, QTextStream & os){
  std::list<Contact *> clist = h->getContacts(b);
  for (std::list<Contact *>::iterator c = clist.begin(); c != clist.end(); c++){

    //transform the contact frame relative to the body to world coordinates,
    //then translate the world coordinates to robot coordinates
    os << h->getTran() * (*c)->getContactFrame() * (*c)->getBody1Tran() << " , ";
  }
}

inline void writeLinkBodyContact(Body * link, Body * b, QTextStream & os){
  os << link->getName() << " ";
  std::list<Contact * > clist = link->getContacts(b);
  os << clist.size() << " ";
  for (std::list<Contact * >::iterator c = clist.begin(); c!=clist.end(); c++){
    os << (*c)->getMate()->getContactFrame() << "  ";

  }
}


//outputs the contacts for each link in the coordinates of the link
void RobotTools::writeContactLinks(Hand * h, Body * b, QTextStream & os){ 
  os << h->getName() << " ";
  writeContactsBody(h, h->getPalm(), os);
  for (int k = 0; k < h->getNumChains(); ++k){
    for (int l = 0; l < h->getChain(k)->getNumLinks(); ++l){
      writeLinkBodyContact(h->getChain(k)->getLink(l), b, os);
      os << " : ";
    }
  }
}
...
//outputs the contacts in the body's fram
void RobotTools::writeContactsBody(Hand * h, Body * b, QTextStream & os){
  std::list<Contact *> clist = h->getContacts(b);
  os << b->getName() << " " << clist.size() << " ";
  for (std::list<Contact * >::iterator c = clist.begin(); c!=clist.end(); c++){
    os << (*c)->getMate()->getContactFrame() << "  ";
  }
}
...
//     DOFParams(const std::vector & <double> paramVec)
//     {      
      
//       if (paramVec.size() < 4)
// 	DBGA("Failed to initialize DOFParams");
//       return;
//     }
//     defaultVelocity = paramVec[0];
//     maxEffort = paramVec[1];
//     Kp = paramVec[2];
//     Kv = paramVec[3];

//     DOF * toDOF()
//     {
//       RigidDOF * newDOF = new RigidDOF();
//       newDOF->setDefaultVelocity(defaultVelocity);
//       newDOF->setMaxEffort(maxEffort);
//       newDOF->setKp(Kp);
//       newDOF->setKv(Kv);
//       newDOF->dofNum = r->getNumDOF();
//       return newDOF;
//     }
...
//   class FingerParams{
//   public:
//     std::vector<DOFParams> dofParamsVec;
//     QString graspitRoot;
//     QString fingerFileName;
//     void addDOFParam(const std::vector<double> & paramVec)
//     {
//       dofParamsVec.append(DOFParams(paramVec));
//     }

//     KinematicChain * addKinematicChain(Robot * r)
//     {
//       TiXmlDocument doc(QString(graspitRoot + fingerFileName));
//       doc.LoadFile();
//       const TiXmlElement * root = doc.RootElement();
//       KinematicChain * finger = new KinematicChain(r, chainNumber, r->getNumJoints());
//       finger->initChainFromXml(root, graspitRoot + "models/robots/EigenHand/iv/", DOFOffset);
//       return finger;
//     }

//     bool loadFinger(Robot * r)
//     {
//       for(int i = 0; i < dofParamsVec.size(); ++i)
// 	{
// 	  r->addDOF(dofParamsVec[i].toDOF());
// 	}
//       addKinematicChain(r);
//       return true;
//     }
...
// Robot * EigenHandTools::createHand(World * w, const std::vector<unsigned int> & chainVector, const std::vector<transf> chainTransf)
// {
...
//     graspitRoot += "/";
//   }
//   QString eigenHandFileName(graspitRoot + "models/robots/EigenHand/EigenHandBase.xml");
//   Robot * newHand = new Robot(w, "EigenHand");
//   return;
// }
