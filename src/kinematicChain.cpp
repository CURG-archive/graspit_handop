...
*/ 
int
KinematicChain::initChainFromXml(const TiXmlElement* root,QString &linkDir, int DOFOffset)
{
	...
	for(p = elementList.begin(), j=0; p!=elementList.end(); p++,j++){
		...
		if(type == "Revolute"){
			...
			if(jointVec[j]->initJointFromXml(*p, firstJointNum+j, 0) == FAILURE ||
			   jointVec[j + 1]->initJointFromXml(*p, firstJointNum+j + 1, 0) == FAILURE){
		...
		if (jointVec[j]->initJointFromXml(*p, firstJointNum+j, DOFOffset) == FAILURE) {
			DBGA("Failed to initialize joint");
			return FAILURE;
		}
	}
