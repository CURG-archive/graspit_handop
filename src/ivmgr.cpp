...
IVmgr::saveImage(QString filename)
{
  ...
  SoOrthographicCamera * orthCam =   dynamic_cast<SoOrthographicCamera *>(myViewer->getCamera());
  ...
  delete myRenderer;
    if(orthCam)
    {
      std::cout << "Camera height: " << orthCam->height.getValue() <<std::endl;
      std::cout << "Viewport Size " << renderArea->getViewportRegion().getViewportSize()[0] << " " << renderArea->getViewportRegion().getViewportSize()[1] << std::endl;
      std::cout << "Viewport Size in Pixels " << renderArea->getViewportRegion().getViewportSizePixels()[0] << " " << renderArea->getViewportRegion().getViewportSizePixels()[1] << std::endl;
      std::cout << "Pixels per inch " << renderArea->getViewportRegion().getPixelsPerInch();
      std::cout << " Pixels per point " << renderArea->getViewportRegion().getPixelsPerPoint() <<std::endl;

   }
    else
      std::cout << "View not orthographic! \n";
}
...
IVmgr::keyPressed(SoEventCallback *eventCB)
{
  ...
  std::cout<< "Something happened !  Ahh\n";
