...
void
Body::getGeometryVerticesInWorldCoordinates(std::vector<position> *vertices) const
{
  std::vector<position>::iterator lastElem;
  ...
  for(int lastElem = prevSize; lastElem < vertices->size(); lastElem ++)
    (*vertices)[lastElem] = (*vertices)[lastElem]*getTran();
