...
class GraspAnalyzingTask :public QThread, public Task {
  ...
  unsigned int graspNum;
  int waitLength;
  unsigned int LimitFlags;
  inline double analyzePerturbedState(GraspPlanningState * p,  const transf &perturbation, 
				      SearchEnergy * se, QTextStream & os, bool use_dynamics = false);
  ...
  inline bool outputInitialPositions(GraspPlanningState * p,  const transf &perturbation,  QTextStream & os);
 public:
