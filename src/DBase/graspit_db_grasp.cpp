...
QString GraspitDBGrasp::getHandDBName(Hand* h)
{
	...
	if(handName == QString("Barrett")){
		...
		} else if(material == h->getWorld()->getMaterialIdx("glass")){
			hand_db_name = QString("NEWBARRETT_GLASS");
		}
	...
	}else if (handName == QString("Eigenhand")){
	  hand_db_name = QString("EIGENHAND");
	}else if(handName == QString("mekaHand")){
	  hand_db_name = QString("MEKA");
	} 
	else {
	  bool ok;
	  handName.toInt(&ok);
...
QString GraspitDBGrasp::getHandGraspitPath(const QString & handDBName)
{
	...
	if (handDBName=="BARRETT_RUBBER") {
		...
		path = "/models/robots/Barrett/Barrett.xml";
	} else if (handDBName=="NEWBARRETT_GLASS") {
		...
		path = "/models/robots/NewBarrett/NewBarrett.xml";
	} else if (handDBName == "EIGENHAND") {
	 	path = "/models/robots/Eigenhand/Eigenhand.xml";
	} else if (handDBName == "MEKA") {
	 	path = "/models/robots/MekaHand/mekaHand.xml";
	} else {
