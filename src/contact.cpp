...
inline transf calculateFrame(const position &pos, const vec3 & normal)
{
  ...
  transf frame = coordinate_transf(pos,tangentX,tangentY);			 
  return frame;
}
...
Contact::Contact(Body *b1, Body *b2, position pos, vec3 norm)
{
  ...
  tangentY = normalise(normal * tangentX);  
  frame = calculateFrame(loc,normal);			 
  coneMat = NULL;
  prevBetas = NULL;
...
void
Contact::setPosition(position newLoc)
{
  loc = newLoc;
  frame = calculateFrame(loc, normal);
}
...
void SoftContact::getStaticContactInfo(std::vector<position> &pVec,std::vector<double> &floatVec){
	cpd.distributionSamples(majorAxis, minorAxis, 10, 10, pVec );
	double params[2] = {majorAxis, minorAxis};
	cpd.sampleForces(params, majorAxis, minorAxis, 10, 10, pVec, floatVec);
}
...
/*!	This computes the wrenches of the virtual contact as used for grasp 
	...
	have been preset depending only on the set of virtual contacts that 
