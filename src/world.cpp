...
World::loadFromXml(const TiXmlElement* root,QString rootPath)
{
	...
	if (!cameraFound && !graspItGUI->useConsole()) {
...
void World::cloneWorldHandSlot(Hand * h, Hand ** handptrptr ){
	
	if (h->isA("Barrett")) {
	  (*handptrptr) = new Barrett( h->getWorld(), "Barrett clone");
	} else if (h->isA("Pr2Gripper")) {
	  (*handptrptr) = new Pr2Gripper( h->getWorld(), "PR2 Gripper clone");
	} else {
	  (*handptrptr) = new Hand(h->getWorld(), "Hand clone");
	}
	emit worldClonedHand();

}

void examineWorldThread(World * w){
 QThread * qt = w->thread();
 if(!qt->isRunning())
     DBGA("The world's thread isn't running!");
 QThread::Priority p = qt->priority();
 if(p==QThread::IdlePriority)
     DBGA("THREAD PRIORITY IS IDLE");
 qt->setPriority(QThread::NormalPriority);
}

Hand * World::cloneHand(Hand * h){
  QEventLoop qe;
  Hand * newHand = NULL;
  connect(this, SIGNAL(worldClonedHand()), &qe, SLOT(quit()));
  connect(this, SIGNAL(cloneWorldHand(Hand*, Hand**)), this, SLOT(cloneWorldHandSlot(Hand*, Hand **)));
  examineWorldThread(this);
  this->thread()->yieldCurrentThread();
  QTimer ss;
  connect(&ss,SIGNAL(timeout()),&qe,SLOT(quit()));
   ss.start(2000);
   ss.setSingleShot(true);
   emit cloneWorldHand(h, &newHand);
  //qe.exec();
  while(!newHand && ss.isActive())
    this->thread()->yieldCurrentThread();
  if(!newHand)
    DBGA("World::cloneHand failed");
