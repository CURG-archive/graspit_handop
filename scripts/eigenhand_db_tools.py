...
def output_grasps_mat(grasp_list):
    ...
    for grasp in grasp_list:
        extended_grasp_joints = list(grasp.grasp_grasp_joints[1:])
        extended_grasp_joints.extend(grasp_joint_extender[:(40 - len(extended_grasp_joints))])
        grasp_descriptor = [grasp.grasp_id, grasp.hand_id, grasp.grasp_iteration, len(grasp.grasp_grasp_joints)] + extended_grasp_joints + [grasp.grasp_energy, grasp.grasp_epsilon_quality] + grasp.grasp_attributes
